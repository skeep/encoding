...
class StatsGenerationError(Exception):
    """Raised when stats generation cannot be completed safely."""
...
def percentile(values: list[float], q: float) -> float:
    ...
    if not values:
        raise StatsGenerationError("Cannot compute percentiles for empty list")
    if len(values) == 1:
        return values[0]
...
def compute_normal(values: list[float], field_name: str) -> dict[str, float | int]:
    ...
    return {
        "distribution": "normal",
        "n": len(values),
        "params": {"mu": mean(values), "sigma": sigma},
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate mixed-distribution stats artifact from canonical CSV."
    )
    parser.add_argument("--input", required=True, help="Path to canonical CSV.")
